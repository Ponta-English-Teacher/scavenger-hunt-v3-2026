...
export async function POST(request:Request){
  try{
    ...
    const upstream=await fetch("https://classroom-question-builder.vercel.app/api/generate-questions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...body,count,followUps:true,includeHints:false})});
    ...
    const items=data.items.map((item,i)=>({id:`q${i+1}`,text:String(item.text||"").trim(),followUp:String(item.followUp||"").trim(),hint:"",grammarTag:String(item.grammarTag||"").trim()})).filter(item=>item.text);
